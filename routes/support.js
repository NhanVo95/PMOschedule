...
const title = "LỊCH ĐĂNG KÝ HỖ TRỢ TRUYỀN THÔNG";
const link = "support";
  ...
  .get(async function (req, res, next) {
    ...
    if (!sess.userid || !(user.length != 2)) {
      ...
      if (user.permission == "admin") {
        switch (query.p) {
          case "addevent":
            res.render("support_adminAddEvent", {
          ...
          case "listevent":
            switch (query.n) {
              case "all":
                res.render("support_adminListEvent", {
                  name: "admin " + user.name,
                  status: "all",
                  title: title,
                  link: link,
              ...
              default:
                res.redirect("/support?p=listevent&n=all");
                break;
            }
            break;
          ...
          case "account":
            switch (query.n) {
              ...
              default:
                res.redirect("/support?p=account&n=all");
                break;
            }
            break;

          default:
            res.render("support_adminSchedule", {
              name: "admin " + user.name,
              title: title,
              link: link,
            });
        ...
        switch (query.p) {
          case "schedule":
            res.render("support_signedSchedule", {
              name: user.name,
              title: title,
              link: link,
            });
          ...
          case "addevent":
            res.render("support_signedAddEvent", {
              name: user.name,
              title: title,
              link: link,
            });
          ...
          case "listevent":
            switch (query.n) {
              case "waiting":
                res.render("support_signedListEvent", {
                  name: user.name,
                  status: false,
                  title: title,
                  link: link,
          ...
          default:
            res.redirect("/support?p=schedule");
            break;
        }
